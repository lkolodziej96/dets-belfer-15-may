    ...
    if (viewState.type === 'sector' && viewState.sector) {
      if (viewState.sector === 'ai') {
        return Object.entries(subsectorDefinitions[viewState.sector]).map(([key, name]) => ({
          ...
          weight:
            weights[key] ??
            defaultAISubsectorWeights[key as keyof typeof defaultAISubsectorWeights] ??
            0,
        }));
        ...
        return Object.entries(subsectorDefinitions[viewState.sector]).map(([key, name]) => ({
          ...
          weight:
            weights[key] ??
            defaultQuantumSubsectorWeights[key as keyof typeof defaultQuantumSubsectorWeights] ??
            0,
        }));
        ...
        return Object.entries(subsectorDefinitions[viewState.sector]).map(([key, name]) => ({
          ...
          weight:
            weights[key] ??
            defaultBiotechSubsectorWeights[key as keyof typeof defaultBiotechSubsectorWeights] ??
            0,
        }));
        ...
        return Object.entries(subsectorDefinitions[viewState.sector]).map(([key, name]) => ({
          ...
          weight:
            weights[key] ??
            defaultSpaceSubsectorWeights[key as keyof typeof defaultSpaceSubsectorWeights] ??
            0,
        }));
  ...
  return (
    <div className="space-y-4">
      <div className={`p-3 rounded-md ${statusInfo.bgColor}`}>
        ...
        <div className={`text-xs mt-1 ${statusInfo.textColor}`}>{statusInfo.message}</div>
