...
const WorldMap: React.FC<Props> = ({ 
  ...
  const zoomToCountries = (countryNames: string[]) => {
    ...
    const features = countryNames.map(countryName => {
        ...
        if (countryDataMap.has('Singapore')) {
          ...
          g.append('circle')
            ...
            .attr('fill', () => {
              let score;
              if (viewState.type === 'sector' && viewState.sector) {
                const sectorScores = Object.values(singaporeData.sectorDetails?.[viewState.sector] ?? {});
            ...
            .style('opacity', selectedCountries.length && !selectedCountries.includes('Singapore') ? 0.5 : 1)
  ...
  }, [data, selectedSector, selectedCountries, onCountrySelect, viewState, countryNameMap]);
  ...
  const generateTooltipContent = (countryData: CountryData, viewState: ViewState, selectedSector: string | null) => {
    let tooltipContent = `
    ...
    const totalScore = viewState.type === 'sector' && viewState.sector
      ? Object.values(countryData.sectorDetails?.[viewState.sector] ?? {})
